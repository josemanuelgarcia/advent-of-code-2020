...
object Day7 extends Puzzle[Seq[String], Int, Int]{
  ...
  override def part1(inputs: Seq[String]): Int = {
    ...
    searchOccurences(graph, "shiny gold")
  }
  ...
  private def parseInput(inputs: Seq[String]): Map[String, Set[(String, Int)]] = {
    inputs.map(_.split("bags contain ")).map(row => {
      ...
      val bagsCointained = appearencesRegex.findAllMatchIn(row(1)).toSeq
      val map = bagsCointained.map(bag => {
  ...
  private def searchOccurences(graph: Map[String, Set[(String, Int)]],initialState: String): Int = {
